...
public class MeleeEnemyBehavior : Target {

	public float Range { get { return kRange; } }
	private const float kRange = 10f; // sword range
	
	private const float kMaxChargeTime = 1f;
	private const float kMinChargeTime = 2.5f;
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		if (other.gameObject.name.Equals("MeleeUnit") ||
		    other.gameObject.name.Equals("MeleeUnit(Clone)")) {
	...
	public void ShootAt(Target enemyUnit, Vector3? firingPosition = null)
	{
	...
	private void FireArrow(Target target)
	{
		...
		MeleeBehavior e = (MeleeBehavior) target.GetComponent(typeof(MeleeBehavior));
		e.Damage(1);		
	}
